...
def create_lively_blue_colormap():
    """
    【修改点1 - 核心配色修改】
    创建以蓝色为主，但添加鲜明红、绿、紫的“活泼”配色方案。
    相比之前的深色系，这里的辅助色更亮、更鲜艳。
    """
    colors = [
        '#1976D2', # 基础蓝 (Standard Blue) - 稳住主基调
        '#FF5252', # 鲜亮红 (Vibrant Red) - 增加活力跳跃感
        '#42A5F5', # 中蓝色 (Mid Blue) - 蓝色过渡
        '#66BB6A', # 鲜亮绿 (Vibrant Green) - 清新活泼感
        '#29B6F6', # 亮天蓝 (Light Blue) - 高光提亮
        '#AB47BC', # 鲜亮紫 (Vibrant Purple) - 丰富色彩层次
        '#1E88E5'  # 回归基础蓝 - 确保整体看去蓝色依然是主导色彩
    ]
    # LinearSegmentedColormap 会在这些颜色间自动插值生成渐变色盘
    # 由于蓝色系在列表中占据多数且分布均匀，整体视觉依然会以蓝为主
    return mcolors.LinearSegmentedColormap.from_list('lively_blue_accent', colors, N=256)
...
def draw_refined_luxury_3d_effects(ax, geo):
    """
    重构立体光影。减少外部投影的脏感，增强内部体积感，打造“悬浮陶瓷”效果。
    """
    # --- 配色定义 (极简、干净) ---
...
def generate_lively_panda_wordcloud(filename):
    ...
    wc = WordCloud(
        ...
        stopwords=stopwords,
        colormap=create_lively_blue_colormap(), # 【修改点2 - 应用新配色】
        random_state=42,
        margin=6, 
    ...
    output_filename = 'lively_blue_panda.png'
...
if __name__ == "__main__":
    jieba.initialize()
    generate_lively_panda_wordcloud("PandaAIQ.jsonl")